...
#[allow(unused)]
pub fn set_windows_proxy(enable: u32, proxy_ip: String, mut ignore_ip: String) {
    let mut ip_iter = ignore_ip.split("\\:");
    if enable == 1 {
        ...
        Command::new(format!("networksetup -setproxybypassdomains {} {}", DEVICE_NAME, DEFAULT_IGNORE_IP.to_owned() + " " + &ignore_ip));
    } else {
